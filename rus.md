...
широко используются членами сообщества Node для обмена модулями.
...
чтобы показать, что один-единственный процесс Node (шестигранник в центре) может
...
Цель Node состоит в достижении золотой середины: относительная простота
...
Прежде всего я советовал бы вам установить Node на ваш компьютер. Самый простой
...
В дополнение к `fs` и сетевым модулям, ядро Node содержит ряд других базовых
...
Node либо ждёт окончания текущих операций с файловыми системами/сетями или же
...
Node переменную вроде `callback` или `cb`, можно предположить, что это функция
обратного вызова.
...
означающее её завершение. В процессе ожидания Node может проверить
...
Если вам нужен модуль событий, в Node вы можете воспользоваться так называемым
...
Для изучения потоков в Node есть два отличных ресурса. Один из них —
...
Ядро Node состоит примерно из двух дюжин модулей, некоторые из них более
...
Когда вы в Node вызываете `require('some_module')`, происходит следующее:
...
найденному Node в процессе прохода вверх по дереву директорий.
...
По умолчанию Node пытается загрузить `module/index.js`, когда вы запрашиваете
...
абсолютно не верно! Название пакетного менеджера Node подразумевает, что он
отвечает за управление модулями, которые Node упаковывает для вас в пакеты.
...
Как любой хороший инструмент, Node лучше всего подходит для конкретного
...
подобных вещей с помощью Node является технически возможным, без проблем не
...
обрести интуитивное понимание сильных сторон Node и того, в каких случаях он
...
задачу вроде моделирования бизнес-логики), однако Node веб-фреймворком не
...
Именно поэтому Node всегда пытается выбрать самое простое и универсальное из
...
Node был реализован именно таким образом, вам 100% следует почитать о [структуре
